...
export function ActionLayerApp() {
  ...
  return <main>
    <header className="site-header shell">
      <Link className="brand" href="/"><BrandMark /><span className="product-name">Action Layer</span></Link>
      <div className="header-links"><span className={webmcp.available ? "status-pill live" : "status-pill"}><b>Agent-ready</b><span>{webmcp.available ? `${tools.length} WebMCP tools available` : `${tools.length} WebMCP tools built in`}</span></span><Link href="/diagnostics">Diagnostics</Link></div>
    </header>

    <section className="hero shell">
      <div className="hero-copy"><p className="kicker">Aliigo Action Layer · WebMCP experiment</p><h1 aria-label="Make a business usable by people and their agents. Built for people. Ready for their agents."><span>Built for people.</span><em>Ready for their agents.</em></h1><p className="lede">Approved business knowledge becomes safe, task-specific tools. The agent researches and prepares. The person stays in control.</p></div>
      <div className="idea-diagram" aria-label="Approved business knowledge and bounded actions become WebMCP tools"><p>One trusted action layer</p><div><span>01</span> Approved business knowledge</div><b>+</b><div><span>02</span> Bounded business actions</div><b>→</b><div className="diagram-result"><span>03</span> WebMCP tools an agent can use</div></div>
    </section>

    <section className="experience shell">
      <div className="experience-intro"><div><p className="section-label">Two business experiences</p><h2>One action layer. Two ways to put it to work.</h2></div><BusinessSwitcher /></div>
      <div className="workspace" style={{ "--accent": business.accent } as CSSProperties}>
        <section className="business-panel">
          ...
          <div className="prompt-card"><span className="prompt-icon">✦</span><div><p>Give this task to an agent</p><blockquote>{selected === "luma" ? "I have back tension and want something Friday afternoon under €100. Find an appropriate service and prepare the best option for me." : "I need 500 premium event postcards for next Friday. I have artwork but don’t know which stock to choose. Prepare a quote request."}</blockquote></div></div>
          <div className="simulation-callout"><div><p>See the prepared state</p><span>The preview runs the same bounded handlers in clearly labeled simulation mode.</span></div><button className="primary-button" type="button" onClick={runScenario} disabled={running}>{running ? "Preparing…" : "Simulate agent flow"}</button></div>
          {error && <p className="error-message" role="alert">{error}</p>}
        </section>

        <aside className="shared-panel">
          <div className="panel-title"><div><p>Human review workspace</p><h2>{prepared ? (prepared.status === "confirmed" ? "Action complete" : prepared.kind === "booking" ? "Ready to review booking" : "Quote request ready") : "Ready for an agent"}</h2></div><span className={prepared ? "pulse active" : "pulse"} /></div>
          {!prepared ? <div className="empty-state"><span>↗</span><h3>Prepared actions appear here.</h3><p>The agent can research and prepare the next step. The person reviews it here before anything happens.</p></div>
          : prepared.kind === "booking" ? <div className="ready-card" data-testid="prepared-booking"><p className="ready-label">{prepared.status === "confirmed" ? "Booking confirmed" : "Prepared for your approval"}</p><h3>{prepared.serviceName}</h3><dl><div><dt>When</dt><dd>{prepared.date} · {prepared.time}</dd></div><div><dt>Price</dt><dd>{prepared.price}</dd></div><div><dt>Note</dt><dd>{prepared.note || "No note"}</dd></div></dl><p className="provenance">✓ {prepared.provenance.source} · reviewed {prepared.provenance.lastReviewed}</p>{prepared.status === "ready" && <button className="confirm-button" type="button" onClick={confirmPrepared}>Confirm booking</button>}{prepared.status === "confirmed" && <p className="confirmed-message">Confirmed by you — not silently by the agent.</p>}</div>
          : <div className="ready-card" data-testid="prepared-quote"><p className="ready-label">{prepared.status === "confirmed" ? "Request submitted" : "Prepared for your approval"}</p><h3>{prepared.quantity} {prepared.productName}</h3><dl><div><dt>Specification</dt><dd>{prepared.size} · {prepared.stock}</dd></div><div><dt>Artwork</dt><dd>{prepared.artworkSupplied ? "Customer artwork supplied" : "Design help needed"}</dd></div><div><dt>Needed</dt><dd>{prepared.neededBy}</dd></div><div><dt>Estimate</dt><dd>{prepared.estimate}</dd></div></dl><p className="provenance">✓ {prepared.provenance.source} · reviewed {prepared.provenance.lastReviewed}</p>{prepared.status === "ready" && <button className="confirm-button" type="button" onClick={confirmPrepared}>Review and submit</button>}{prepared.status === "confirmed" && <p className="confirmed-message">Submitted by you — the agent only prepared it.</p>}</div>}
